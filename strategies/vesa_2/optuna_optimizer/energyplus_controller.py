"""EnergyPlus runtime controller — optimized version.
...
# optuna_optimizer is 3 levels deep from project root -> parents[3]
...
class EnergyPlusController:
    ...
    def initialize_handles(self, state: Any) -> None:
        ...
        if bad:
            pass # print(f"WARNING: the following handles resolved to -1: {bad}")
            # Keep silent for excessive spam during parameter tuning
        else:
            pass # print(f"All {len(self.handles)} handles initialised successfully.")
    ...
    def get_api_data(self, state: Any) -> None:
        pass # Optimization wrapper ignores this logic to speed up runtime.
