"""EnergyPlus runtime controller — template version.

Bridges the EnergyPlus Python API with your control model.
Reads sensor values each timestep, passes them to your model,
and writes the resulting setpoints back to EnergyPlus.

You can extend this controller as needed (e.g. add trajectory
logging, normalization, etc.).
"""
...
class EnergyPlusController:
    ...
    def _collect_obs(self, state: Any) -> Dict:
        obs = {
            ...
            "space1_occupancy": self.get_variable("space1_occupancy", state),
            ...
            "space2_occupancy": self.get_variable("space2_occupancy", state),
            ...
            "space3_occupancy": self.get_variable("space3_occupancy", state),
            ...
            "space4_occupancy": self.get_variable("space4_occupancy", state),
            ...
            "space5_occupancy": self.get_variable("space5_occupancy", state),
            ...
            "month": float(self.api.exchange.month(state)),
            "day_of_month": float(self.api.exchange.day_of_month(state)),
        }
        return obs
