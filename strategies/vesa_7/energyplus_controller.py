"""EnergyPlus runtime controller for vesa_7."""
...
class EnergyPlusController:
    ...
    def initialize_handles(self, state: Any) -> None:
        ...
        bad = [name for name, handle in self.handles.items() if handle == -1]
        if bad:
            print(f"WARNING: unresolved handles: {bad}")
        else:
            print(f"All {len(self.handles)} handles initialized.")
    ...
    def _collect_obs(self, state: Any) -> Dict[str, float]:
        obs: Dict[str, float] = {
