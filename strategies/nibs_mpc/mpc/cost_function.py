...
# CO2 penalty per zone per timestep (matches scoring: each 15-min row penalized at full rate)
CO2_THRESHOLDS = np.array([770.0, 970.0, 1220.0])
CO2_PENALTIES = np.array([2.0, 10.0, 50.0])   # €/timestep per zone

# Temperature penalty per zone per timestep
...
def compute_step_cost(
    ...
    for z in range(5):
        ...
        co2_penalty = co2_penalty + p

    # Temperature penalty (per zone)
