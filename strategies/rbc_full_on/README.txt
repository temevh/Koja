RBC Full-On — EnergyPlus Simulation
====================================

Simple full-on baseline controller where all setpoints are fixed and
the fan always runs at maximum capacity.
...
rbc_full_on/, the .idf file, etc.) and run:
...
with the absolute path to your project folder:
  ...
  OUT_DIR  = Path("<PROJECT_ROOT>/rbc_full_on/eplus_out")
...
Make sure the virtual environment is activated, then:
  ...
  cd rbc_full_on          # from the project root
  python run_idf.py
...
.........................
  ...
  2. Navigate to the project root folder (the folder that contains
     rbc_full_on/, drl/, and the .idf file).
...
rbc_model_1.py
--------------
The control strategy. Implements a simple full-on controller where
all setpoints are fixed and the fan always runs at maximum capacity.

  - calculate_setpoints(zone_temp, outdoor_temp, return_air_temp,
                        occupancy, hour, day, co2_concentration)
      Returns fixed values every timestep:
        heating setpoint = 22.5 C
        cooling setpoint = 22.5 C
        supply air temp  = 19.0 C
        fan flow rate    = 1.0 kg/s (FLOW_MAX)

  No outdoor compensation, no scheduling, no CO2 demand control.
  Serves as a simple baseline to compare against smarter strategies.
