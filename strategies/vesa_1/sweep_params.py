...
# EnergyPlus: use pip ``pyenergyplus-lbnl`` (``pip install -r requirements.txt``).
...
CONTROLLER_SRC = STRATEGIES / "rbc_scheduled" / "energyplus_controller.py"
...
PARAM_GRID = {
    # CO2 Control Band
    "CO2_MIN_LIMIT":          [600, 700],
    "CO2_MAX_LIMIT":          [800, 950, 1050],
    
    # Fan Flow Control
    "FLOW_LOW":               [0.10, 0.15],
    "FLOW_MODERATE":          [0.30, 0.40],
    
    # Supply Air Strategy when warm
    "SUP_TEMP_AT_HIGH":       [17.0, 19.0],
}

# These parameters stay fixed for all runs (not swept):
FIXED_PARAMS = {
    # Based on the earlier best run 8280.65€
...
OUTDOOR_TEMP_LOW_LIMIT = {p["OUTDOOR_TEMP_LOW_LIMIT"]}
OUTDOOR_TEMP_HIGH_LIMIT = {p["OUTDOOR_TEMP_HIGH_LIMIT"]}
