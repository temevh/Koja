"""Optuna optimization for vesa_5 with auto-persisted best parameters."""
...
def load_eplusout(csv_path: Path) -> pd.DataFrame:
    ...
    # Keep Date/Time as canonical first column so timestep detection stays reliable.
    # EnergyPlus exports may vary slightly in header naming/width depending on run config.
    ...
    if len(df.columns) >= 2:
        # Rename known metric columns positionally, preserving Date/Time in column 0.
...
def objective(trial: optuna.Trial) -> float:
    ...
    params = {
        "htg_margin": trial.suggest_float("htg_margin", 0.1, 0.9),
        "clg_margin": trial.suggest_float("clg_margin", 0.1, 0.9),
        "night_htg_setback": trial.suggest_float("night_htg_setback", 0.3, 1.8),
        "night_clg_setup": trial.suggest_float("night_clg_setup", 0.3, 1.8),
        "precondition_hours": trial.suggest_float("precondition_hours", 0.25, 2.5),
        "sup_temp_cold": trial.suggest_float("sup_temp_cold", 17.0, 21.0),
        "sup_temp_warm": trial.suggest_float("sup_temp_warm", 16.0, 19.5),
        "return_air_cold_ref": trial.suggest_float("return_air_cold_ref", 20.0, 23.0),
        "return_air_warm_ref": trial.suggest_float("return_air_warm_ref", 23.0, 26.0),
        "co2_low_threshold": trial.suggest_float("co2_low_threshold", 500.0, 700.0),
        "co2_high_threshold": trial.suggest_float("co2_high_threshold", 700.0, 950.0),
        "co2_emergency": trial.suggest_float("co2_emergency", 850.0, 1200.0),
        "flow_min_occupied": trial.suggest_float("flow_min_occupied", 0.08, 0.30),
        "flow_boost": 1.0,
        "flow_night": trial.suggest_float("flow_night", 0.0, 0.10),
        "flow_pre_flush": trial.suggest_float("flow_pre_flush", 0.2, 0.7),
        ...
        "work_end": trial.suggest_float("work_end", 16.5, 19.5),
        "extended_end": trial.suggest_float("extended_end", 20.0, 23.5),
        "min_deadband": trial.suggest_float("min_deadband", 0.5, 2.0),
    }

    if params["co2_high_threshold"] <= params["co2_low_threshold"] + 50.0:
        params["co2_high_threshold"] = params["co2_low_threshold"] + 50.0
    if params["co2_emergency"] <= params["co2_high_threshold"] + 10.0:
        params["co2_emergency"] = params["co2_high_threshold"] + 10.0
    ...
    from my_model import Vesa5Model

    api = EnergyPlusAPI()
    state = api.state_manager.new_state()
    model = Vesa5Model(**params)
    ...
    if costs["total_cost"] < BEST_COST:
        ...
        print("Saved:")
        print(f"  - {BEST_JSON.name}")
        print(f"  - {BEST_PY.name}")
        print(f"  - {BEST_TXT.name}")
        print(f"  - {BEST_RUN_DIR}")
        for key in sorted(params.keys()):
            print(f"  {key} = {_safe_float(params[key])}")
        print("=" * 72)
...
def main() -> None:
    ...
    study = optuna.create_study(
        study_name="vesa5_hvac_optimization",
    ...
    if len(study.trials) > 0:
        try:
            ...
            # Keep reproducibility files in sync with best trial when resuming an existing study.
            ...
            write_best_files(study.best_params, best_costs, study.best_trial.number, float(best_attrs.get("elapsed_s", 0.0)))
        except Exception:
            BEST_COST = float("inf")

    n_trials = 100
    print(f"Starting vesa_5 optimization ({n_trials} trials)")
    print(f"Current best in study: {BEST_COST if np.isfinite(BEST_COST) else 'none'}")
    print("Each time a better trial appears, best parameter files are overwritten automatically.")
