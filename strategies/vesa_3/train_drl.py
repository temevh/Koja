"""
train_drl.py — vesa_3 strategy: DRL (BC + SAC) on the shared DOAS model.

Same pipeline as strategies/drl; outputs and models are isolated under
this folder (vesa_3_output/, models/).
...
# Reuse expert trajectories from strategies/drl, or set to "expert_data.json" after copying.
EXPERT_JSON  = os.path.join("..", "drl", "expert_data.json")
MODEL_DIR    = "models"
TRAIN_OUT    = "vesa_3_output/train"
EVAL_OUT     = "vesa_3_output/train_eval"
...
ACTION_SPEC = {
    ...
    # Must match rbc_scheduled expert_data.json (energyplus_controller action_max).
    # README allows up to 1.0 kg/s; regenerate expert with 1.0 if you change this.
...
WARMUP_BUFFER_STEPS = 576                  # ~6 days @ 96 steps/day; must match hackathon template intent


def full_year_total_steps() -> int:
    """Full calendar year at 15-min zoning timestep + warm-up budget (see eplus_sim)."""
    return 96 * 365 + WARMUP_BUFFER_STEPS


def training_total_steps() -> int:
    """Episode length for RL. Use env VES33_SHORT_DAYS=N for fast Colab dry-runs (N simulated days)."""
    raw = os.environ.get("VES33_SHORT_DAYS", "").strip()
    if raw:
        return 96 * int(raw) + WARMUP_BUFFER_STEPS
    return full_year_total_steps()
...
def _build_config(idf, weather, output_path, phase, total_steps: Optional[int] = None):
    """If *total_steps* is None, use a full hackathon year (+ warm-up buffer)."""
    obs_names = list(OBS_SPEC.keys())
    act_names = list(ACTION_SPEC.keys())
    steps = full_year_total_steps() if total_steps is None else int(total_steps)
    return {
...
def load_expert_pairs(json_path, config):
    if not os.path.isfile(json_path):
        raise FileNotFoundError(
            f"Expert JSON not found: {json_path}\n"
            "Run strategies/rbc_scheduled/run_idf.py (or another RBC) to generate "
            "expert_data.json, or set EXPERT_JSON in train_drl.py."
        )
    with open(json_path, "r", encoding="utf-8") as f:
        data = json.load(f)
    if not data:
        raise ValueError(f"Expert JSON is empty: {json_path}")

    obs_keys = config["observations"]
    act_keys = config["rl_actions"]
    sample = data[0]
    missing_o = [k for k in obs_keys if k not in sample.get("obs", {})]
    missing_a = [k for k in act_keys if k not in sample.get("action", {})]
    if missing_o or missing_a:
        raise KeyError(
            "Expert trajectories do not match OBS_SPEC / ACTION_SPEC. "
            f"Missing obs keys: {missing_o}; missing action keys: {missing_a}."
        )
...
if __name__ == "__main__":
    # Only ONE EnergyPlusEnv may run at a time: eplus_sim.py uses process-wide globals
    # (API, threading.Events, data buffers). Never open two envs concurrently.
    train_steps = training_total_steps()
    train_cfg = _build_config(IDF_FILE, WEATHER_FILE, TRAIN_OUT, "learn", total_steps=train_steps)
    eval_cfg_bc = _build_config(IDF_FILE, WEATHER_FILE, EVAL_OUT, "test", total_steps=train_steps)

    train_env = Monitor(EnergyPlusEnv(train_cfg))
    eval_env = Monitor(EnergyPlusEnv(eval_cfg_bc))

    os.makedirs(MODEL_DIR, exist_ok=True)

    bc_epochs = int(os.environ.get("VES33_BC_EPOCHS", "50"))
    sac_total = int(os.environ.get("VES33_SAC_STEPS", "300000"))
    sac_freeze = int(os.environ.get("VES33_SAC_FREEZE_STEPS", "5000"))
    skip_bc_eval = os.environ.get("VES33_SKIP_BC_EVAL", "").strip() in ("1", "true", "yes")

    # --- Step 1: Behavioral Cloning ---
    obs, acts = load_expert_pairs(EXPERT_JSON, train_cfg)
    ...
    # --- Step 2: Evaluate BC policy (optional; each run is one full EnergyPlus episode) ---
    if not skip_bc_eval:
        print("Evaluating BC policy in EnergyPlus...")
    ...
    # Checkpoint only — EvalCallback would need a second concurrent EnergyPlusEnv, which
    # is unsafe with the current global-state EnergyPlus bridge.
    chk_freq = int(os.environ.get("VES33_CHECKPOINT_FREQ", "50000"))
    # Ensure at least one checkpoint mid-episode when a year is shorter than chk_freq.
    save_every = min(chk_freq, max(2000, train_steps // 2))

    checkpoint_cb = CheckpointCallback(
        save_freq=save_every,
        save_path=MODEL_DIR,
        name_prefix="sac_bc_hvac_ckpt",
        verbose=1,
    )
    ...
    sac_model.learn(total_timesteps=sac_freeze)

    # Unfreeze and run full training
    for p in sac_model.policy.actor.parameters():
        p.requires_grad = True
    sac_model.learn(total_timesteps=sac_total, callback=checkpoint_cb, progress_bar=True)
