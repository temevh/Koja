...
BASE_OUT_DIR = Path(r"sweep_output")
...
def run_simulation(params: Dict, run_id: int, total: int) -> Dict:
    ...
    # Copy supporting files into the temp directory
    for f in ["energyplus_controller.py", "variable_config.py"]:
        src = Path(f)
        if src.exists():
            shutil.copy2(src, TEMP_RUN_DIR / f)

    # We need to import the generated model from the temp directory
    ...
    if 'variable_config' in sys.modules:
        del sys.modules['variable_config']
